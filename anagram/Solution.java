		...
		if (s.length() % 2 == 0) {
			for (int i = 0; i < s1.length(); i++) {
				...
				if (sb.indexOf(String.valueOf(temp)) == -1) {
					...
					sb.delete(s2.indexOf(String.valueOf(temp)), s2.indexOf(String.valueOf(temp))+1);
...
//		bufferedWriter.close();
		String s = "njdtcezqezsjbgotwjtacwngzecihkxvtaqxepnkqiusoklnvepyceebiijqsinnkfkpu";
		System.out.println(Result.anagram(s));
	}
}
 ...
 * the substrings are contiguous and their lengths are equal. Now you can change
 * 'a' and 'b' in the first substring to 'd' and 'e' to have 'dec' and 'cde'
 ...
 * 
 * int: the minimum number of characters to change or -1. Input Format
 * 
 * The first line will contain an integer, , the number of test cases. Each test
 * case will contain a string .
 * 
 * Constraints
 * 
 * 
 * consists only of characters in the range ascii[a-z]. Sample Input
 * 
 * 6 aaabbb ab abc mnop xyyx xaxbbbxx Sample Output
 * 
 * 3 1 -1 2 0 1 Explanation
 * 
 ...
 * 
 * Test Case #06: Here S1 = "xaxb" and S2 = "bbxx". You must replace 'a' from S1
 * with 'b' so that S1 = "xbxb".
 */
